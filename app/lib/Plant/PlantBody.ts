...
class PlantBody {
    ...
    init(): void {
        ...
        tree.eachSegment((n0: node, n1: node): void => {
            ...
            const leaves = this.genus.getLeaves(n0, n1);
            leaves.map((cfg: leafDefinition): void => {
    ...
    private render(age: number, colors = true, svg: SVGElement): void {
        ...
        const getRandomColor = (): string => {
            const randomIndex = Math.floor(Math.random() * Randcolors.length);
            return Randcolors[randomIndex];
        };
        ...
        if (this.parts) for (const p of this.parts) {
            ...
            if (p instanceof Leaf) {
                //FOUND: this is where you change the leaf color.
